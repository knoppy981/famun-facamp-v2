...
function StepPreview({ step, direction }: { step: Step; direction: 1 | -1 }) {
  ...
  return (
    <motion.div
      ...
      className="relative h-full w-full overflow-hidden rounded-sm rounded-rb-lg rounded-tl-xl "
    >
      {step.media ? (
        ...
        <div className="flex h-full items-center justify-center p-6">
          ...
          >
            <h3 className="mb-2 text-2xl text-primary">
              {step.title}
            </h3>
...
function StepTab({ step, order, isActive, onClick, isCompleted }: StepTabProps) {
  return (
    <motion.button
      onClick={onClick}
      className={cn(
    ...
    >
      <div className="mb-1 text-sm font-medium">{order}. {step.title}</div>
...
export function IntroDisclosure({
  ...
  if (isDesktop) {
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        >
          <DialogHeader className="p-6 space-y-2 border-b border-border">
            <DialogTitle>Como funciona nossa inscrição?</DialogTitle>
            {showProgressBar && (
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerContent className="h-[95svh] sm:h-[95vh] max-h-[95vh] ">
        <motion.div
          drag="x"
        ...
        >
          <DrawerHeader className="text-left  pb-4 space-y-4">
            ...
            <DrawerTitle>Como nossa inscrição funciona?</DrawerTitle>
